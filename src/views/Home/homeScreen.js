...
import { getTerminalServingType, getPriceRangeCurrencyType } from '../../actions/homeScreenAction';
...
class HomeScreen extends Component {
    ...
    sortSelection() {
        ...
        this.setState({ filterType: '', filterSelected: 'selected', filter: { ...this.state.filter, sort } });
    ...
    rangeSelection() {
        ...
        var newRange = [];
        newRange.push(this.state.priceRange)

        this.setState({ filter: { ...this.state.filter, range: newRange }, filterType: '', filterSelected: 'selected' });
        this.doSpecialFoodType();
    }
    dietarySelection() {
        ...
        this.setState({ filter: { ...this.state.filter, "pure_veg": this.state.dietary }, filterType: '', filterSelected: 'selected' })
    ...
    dietarySelected() {
        ...
        this.setState({ dietary: this.state.dietary === 'yes' ? "" : 'yes' })
    ...
    sortSelected(value) {
        ...
        this.setState({ sortVar: 'done', sort: value })
        //this.setState({ filter: { ...this.state.filter, sort }, sortVar: 'done',sort: value})
    ...
    disableDateTimePopup() {
        this.setState({ isAskDateTime: false, outletId: '', indexFor: '', itemData: '', selectedDate: '', selectedTime: '' });
        storeData('dateForOutlet', '');
    }
    ...
    foodSelection() {
        ...
        this.setState({ filter: { ...this.state.filter, specail }, filterType: '', filterSelected: 'selected' })
        ...
        getData('user').then((user) => {
            ...
            getData('accessToken').then((accessToken) => {
                getData('selectedAirport').then((selectedAirport) => {
                    ...
                    getPriceRangeCurrencyType(this.props.dispatch, data).then((response) => {
                        if (response && response.message && response.message === 'Please provide valid access token') {
                            clearData();
                            Alert.alert("Token Expire please login again.")
                            this.props.navigation.navigate("Auth")
                        ...
                        if (response.status) {
                            console.log('getPriceRangeCurrencyType: ' + JSON.stringify(response))
                            var arrData = response.priceRangeCurrency
                            var IndexItem = arrData[0]
                            storeData('currencyType', IndexItem.currency_name);

                            console.log("currency_name : " + IndexItem.currency_name)
                            this.setState({ currencyName: IndexItem.currency_name })
                            // this.doSpecialFoodType();
                        }
                    });
                })
    ...
    onConfirm(hour, minute) {
        ...
        if (this.state.selectedDate && this.state.selectedDate.length > 0) {
            storeData('dateForOutlet', this.state.selectedDate + ' ' + `${hour}:${minute}`);
            this.setState({ dateTimeForOutlet: this.state.selectedDate + ' ' + `${hour}:${minute}` });
        }
        this.TimePicker.close();
    }
    render() {
                ...
                (this.state.selectedServingTypeId && this.state.selectedServingTypeId === '2') ?
                    ...
                    (this.state.selectedServingTypeId && this.state.selectedServingTypeId === '3') ?
                        ...
                        (this.state.selectedServingTypeId && this.state.selectedServingTypeId === '4') ?
                            ...
                            :
                            (this.state.selectedServingTypeId && this.state.selectedServingTypeId === '5') ?
                                data = this.state.data.homeData.Within_security
                                :
                                data = undefined
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                <View style={{ height: '100%', width: '100%', flex: 1, flexDirection: 'column' }}>
                    <View style={{
                        ...
                        <View style={{ flex: 0.2, flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Filters', { onGoBack: (filters) => this.refresh(filters), filter: this.state.filter, 'currencyName': this.state.currencyName })}>
                                <Image
                        ...
                        <View style={{ height: 50, width: '100%', marginTop: 0, alignItems: 'center', justifyContent: 'center', alignSelf: 'center' }}>
                            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                                ...
                                <TouchableOpacity onPress={() => this.setState({ filterType: 'Dietary' })} style={{
                                    ...
                                    justifyContent: 'center',
                                    alignSelf: 'center',
                                    backgroundColor: this.state.filter.pure_veg ? '#F6512B' : '#F5F5F5',
                                    borderRadius: 25
                                }}>
                                    <CustomText text={'Dietary'} style={{
                                        fontSize: getDimen(.040),
                                        color: this.state.filter.pure_veg ? 'white' : 'black',
                                    }}></CustomText>
                                    <Image
                                        style={{ width: 15, height: 15, resizeMode: 'stretch', marginLeft: 10, tintColor: this.state.filter.pure_veg ? 'white' : 'black' }}
                                ...
                                <TouchableOpacity onPress={() => this.setState({ filterType: 'Range' })} style={{
                                    ...
                                    justifyContent: 'center',
                                    alignSelf: 'center',
                                    backgroundColor: (this.state.filter.range && this.state.filter.range.length > 0) ? '#F6512B' : '#F5F5F5',
                                    borderRadius: 25
                                }}>
                                    <CustomText text={'Price Range'} style={{
                                        fontSize: getDimen(.040),
                                        color: (this.state.filter.range && this.state.filter.range.length > 0) ? 'white' : 'black',
                                    }}></CustomText>
                                    <Image
                                        style={{ width: 15, height: 15, resizeMode: 'stretch', marginLeft: 10, tintColor: (this.state.filter.range && this.state.filter.range.length > 0) ? 'white' : 'black' }}
                                        source={require('../../assets/images/down-arrow.png')}
                                    />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => this.setState({ filterType: 'Sort' })} style={{
                                    ...
                                    justifyContent: 'center',
                                    alignSelf: 'center',
                                    backgroundColor: (this.state.filter.sort && this.state.filter.sort.length > 0) ? '#F6512B' : '#F5F5F5',
                                    borderRadius: 25
                                }}>
                                    <CustomText text={'Sort'} style={{
                                        fontSize: getDimen(.040),
                                        color: (this.state.filter.sort && this.state.filter.sort.length > 0) ? 'white' : 'black',
                                    }}></CustomText>
                                    <Image
                                        style={{ width: 15, height: 15, resizeMode: 'stretch', marginLeft: 10, tintColor: (this.state.filter.sort && this.state.filter.sort.length > 0) ? 'white' : 'black' }}
                                        source={require('../../assets/images/down-arrow.png')}
                                    />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => this.setState({ filterType: 'Food', specialFood: (this.state.filter.specail && this.state.filter.specail.length > 0) ? this.state.filter.specail[0] : "" })} style={{
                                    flexDirection: 'row',
                                    marginLeft: 10,
                                    paddingLeft: 10,
                                    paddingRight: 10,
                                    ...
                                    justifyContent: 'center',
                                    alignSelf: 'center',
                                    backgroundColor: (this.state.filter.specail && this.state.filter.specail.length > 0) ? '#F6512B' : '#F5F5F5',
                                    borderRadius: 25
                                }}>
                                    <CustomText text={'Special Food'} style={{
                                        fontSize: getDimen(.040),
                                        color: (this.state.filter.specail && this.state.filter.specail.length > 0) ? 'white' : 'black',
                                    }}></CustomText>
                                    <Image
                                        style={{ width: 15, height: 15, resizeMode: 'stretch', marginLeft: 10, tintColor: (this.state.filter.specail && this.state.filter.specail.length > 0) ? 'white' : 'black' }}
                                        source={require('../../assets/images/down-arrow.png')}
                                    />
                                </TouchableOpacity>
                    ...
                    <ScrollView style={{ flex: .8, width: '100%' }}>
                        <View>
                            {this.state.servingtypeArr && this.state.servingtypeArr.length>0 && <View style={{ paddingTop: 10, paddingBottom: 10, width: '100%', flexDirection: 'row' }}>
                                {this.state.servingtypeArr.map((item, i) => {
                                    return (
                                        <View key={item.id} style={{ marginLeft: 10, marginTop: 3, marginRight: 10, marginBottom: 3, justifyContent: 'center' }}>
                            ...
                            {(data && data.user_fav && data.user_fav.length > 0) ?
                                <View style={{ height: getDimen(.50), marginTop: 10, flexDirection: 'column', marginLeft: 10 }}>
                                    ...
                                    <CustomText font={'medium'} text={'Your Favorites'} style={{ fontSize: getDimen(.040), marginTop: 5, flex: 0.15, paddingLeft: 3, marginBottom: 5 }} />
                                    ...
                                    <FlatList
                                        ...
                                        data={data.user_fav}
                                        keyExtractor={(item, index) => "userFav" + index}
                                        renderItem={({ item, separators, index }) => (
                                            <TouchableWithoutFeedback onPress={() => this.handlerSimpleCall(index, item)}>

                                                <View style={{ flexDirection: "column", width: getDimen(1) / 2 }}>
                                                    <View style={{ flexDirection: "column", flex: 1, borderRadius: 2, borderColor: '#EAEAEA', borderWidth: 1, marginLeft: 10 }}>
                                                        ...
                                                        <View style={{ flexDirection: "column", flex: 0.75, }}>
                                                            <View style={{ flexDirection: "column", flex: 1 }}>
                                                                ...
                                                                <View style={{ flex: .70, alignItems: 'center', justifyContent: 'center', paddingLeft: 5, paddingRight: 5 }}>
                                                                    <Image
                                                                ...
                                                                <View style={{ flex: .30, flexDirection: 'column' }}>
                                                                    ...
                                                                    <CustomText font={'regular'} text={item.cuisinies_name} style={{ alignItems: 'flex-start', justifyContent: 'center', alignSelf: 'flex-start', fontSize: getDimen(.020), marginLeft: 10 }} />
                                                        ...
                                                        <View style={{ flexDirection: "row", flex: 0.25, }}>
                                                            <View style={{ flexDirection: "row", flex: 1, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', }}>
                                                                ...
                                                                {item.average_rating ?
                                                                    <View style={{ flexDirection: "row", marginLeft: 10, marginRight:10,flex: 0.20, backgroundColor: '#48C479', alignSelf: 'center', paddingLeft: 5, paddingRight: 5 }}>
                                                                        <Image
                                                                            style={{ width: 10, height: 10, resizeMode: 'contain', alignSelf: 'center' }}
                                                                            source={require('../../assets/images/home-star.png')}
                                                                        />
                                                                        <CustomText font={'regular'} text={item.average_rating} style={{ fontSize: getDimen(.025), paddingLeft: 3,color: 'white', alignSelf: 'center' }} />
                                                                ...
                                                                {item.offer_description ?
                                                                    <View style={{ flex: 0.70, flexDirection: "row", alignItems: 'center', justifyContent: 'center', alignSelf: 'center', }}>
                                                                        ...
                                                                        />
                                                                        <CustomText font={'regular'} text={item.offer_description} style={{ alignSelf: 'center', fontSize: getDimen(.025), paddingLeft: 2, paddingRight: 2 }}></CustomText>
                                                                    </View>
                                                                    :
                                                                    <View style={{ flex: 0.70, flexDirection: "row", alignItems: 'center', justifyContent: 'center', alignSelf: 'center', }}>
                                                                    </View>
                            ...
                            {(data && data.latest_delas.length > 0) ?
                                <View style={{ height: getDimen(.45), marginTop: 10, flexDirection: 'column', marginLeft: 10 }}>
                                    ...
                                    <FlatList
                                        horizontal={true}
                                        showsHorizontalScrollIndicator={false}
                                        style={{ flex: 0.75, paddingLeft: 0, marginTop: 10, marginRight: 10, marginLeft: 10 }}
                                        ...
                                        renderItem={({ item, separators, index }) => (
                                            <TouchableWithoutFeedback onPress={() => this.props.goToOutletList(item.id)
                                                ...
                                                <View style={{
                                                    ...
                                                    <View style={{ flex: 1, flexDirection: 'column', width: getDimen(1) / 1.85 }}>
                                                        <Image
                                                            resizeMode="cover"
                            ...
                            {(data && data.dealof_theday.length > 0) ?
                                <View style={{ height: getDimen(.5), marginTop: 10, flexDirection: 'column', marginLeft: 10 }}>

                                    <CustomText font={'medium'} text={" Deal Of The Day "} style={{ fontSize: getDimen(.040), marginTop: 10, flex: 0.20, }}></CustomText>

                                    <FlatList
                                        horizontal={true}
                                        showsHorizontalScrollIndicator={false}
                                        style={{ flex: 0.80, marginRight: 10 }}
                            ...
                            {(data && data.outlet_list && data.outlet_list.length > 0) ?
                                <View style={{ marginTop: 10, marginLeft: 10, marginBottom: 10 }}>
                                    ...
                                    {
                                        data.outlet_list.map((item, i) => {
                                            return (
                                                <TouchableWithoutFeedback
                                                    key={"outlet_list" + i}
                                                    onPress={() => this.handlerSimpleCall(i, item)}>

                                                    <View style={{ height: getDimen(.28), marginLeft: 10 }}>
                                                        <View style={{ flex: 1, flexDirection: "row", marginLeft: 0, marginRight: 15, marginTop: 10, marginBottom: 10 }}>
                                                            ...
                                                            <View style={{ flex: .65, flexDirection: "column", backgroundColor: 'white' }}>
                                                                ...
                                                                <View style={{ flexDirection: 'column', flex: 0.85 }}>
                                                                    ...
                                                                    {item.offer_description ?
                                                                        <View style={{ flexDirection: "row", paddingLeft: 10, marginTop: 2, marginBottom: 2 }}>
                                                                            <Image
                                                                                style={{ width: 15, height: 15, resizeMode: 'contain', alignSelf: 'center' }}
                                                                                source={require('../../assets/images/home-offer.png')}
                                                                            />
                                                                ...
                                                                {item.average_rating ?
                                                                    <View style={{ flexDirection: 'row', marginLeft: 5, flex: 0.15, }}>
                                                                        ...
                                                                        {/* <CustomText text={this.getRatingText(item.average_cost_per_person, item.currency_name)} style={{ fontSize: getDimen(.025), marginLeft: 50 }}></CustomText> */}
                                                                        <Text style={{ fontSize: getDimen(.025), marginLeft: 50 }}>{this.getRatingText(item.average_cost_per_person, item.currency_name)}</Text>
                ...
                {(data && data.outlet_list && data.outlet_list.length > 0) ?
                    ...
                    <View style={{ position: 'absolute', width: 60, height: 60, bottom: 10, right: 10 }}>
                        <Image source={require('../../assets/images/orange.png')} style={{ width: 60, height: 60, resizeMode: 'contain', alignSelf: 'center' }} />

                        <TouchableOpacity
                            style={{ width: 60, height: 60, justifyContent: 'center', position: "absolute" }}
                            onPress={() => this.props.navigation.navigate("Offers")}>
                            <Image source={require('../../assets/images/offer.png')} style={{ width: 20, height: 20, resizeMode: 'contain', alignSelf: 'center', tintColor: 'white' }} />
                ...
                {/* {(this.state.filterType === 'Dietary') ?
                    <Modal isVisible={true}>
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute' }}>
                            <TouchableOpacity style={{ flex: .75, backgroundColor: 'transparent' }} onPress={() => this.setState({ filterType: '' })}></TouchableOpacity>
                ...
                {(this.state.filterType === 'Dietary') ?
                    ...
                    <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: '' }}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute', backgroundColor: 'transparent', }}>
                            <TouchableOpacity style={{ flex: .75, }} onPress={() => this.setState({ filterType: '', dietary: this.state.filter.pure_veg })}></TouchableOpacity>
                            <View style={{ flex: .25, backgroundColor: 'white' }}>
                                <View style={{ flex: 1, flexDirection: 'column', marginLeft: 25, marginRight: 10, marginTop: 10, marginBottom: 10, }}>
                                    <CustomText font={'medium'} text={'Dietary'} style={{ flex: 1, color: 'black', fontSize: getDimen(.055), marginTop: 10, }}></CustomText>
                                    <TouchableOpacity style={{ flex: 1, flexDirection: 'row', }} onPress={() => this.dietarySelected()}>
                                    ...
                                    {console.log("this.state.filter.pure_veg : " + this.state.filter.pure_veg)}
                                    <View style={{ flex: 1, height: '100%', width: '100%', justifyContent: 'center', alignSelf: 'center', alignItems: 'center', }}>
                                        <TouchableOpacity
                                            style={styles.signup}
                                            onPress={() => this.dietarySelection()}>
                ...
                {(this.state.filterType === 'Range') ?
                    <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: '' }}>
                        ...
                        <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: 'black', opacity: 0.5 }}></View>
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute', backgroundColor: 'transparent', }}>
                            <TouchableOpacity style={{ flex: .70, }} onPress={() => this.setState({ filterType: '', priceRange: this.state.filter.range ? this.state.filter.range[0] : "" })}></TouchableOpacity>
                            <View style={{ flex: .30, backgroundColor: 'white' }}>
                                <View style={{ flex: 1, flexDirection: 'column', marginLeft: 25, marginRight: 10, marginTop: 1, marginBottom: 1, }}>
                                    ...
                                    <View style={{ flex: 0.3, height: '100%', width: '100%', justifyContent: 'center', alignSelf: 'center', alignItems: 'center', marginBottom: 5 }}>
                                        <TouchableOpacity
                                            style={styles.signup}
                ...
                {(this.state.filterType === 'Sort') ?
                    <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: '' }}>
                        ...
                        <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: 'black', opacity: 0.5 }}></View>
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute', backgroundColor: 'transparent', }}>
                            <TouchableOpacity style={{ flex: .70, }} onPress={() => this.setState({ filterType: '', sort: this.state.filter.sort ? this.state.filter.sort[0] : "" })}></TouchableOpacity>
                            <View style={{ flex: .30, backgroundColor: 'white', }}>
                                <View style={{ flex: 1, flexDirection: 'column', marginLeft: 25, marginRight: 10, marginTop: 10, marginBottom: 10, }}>
                                    ...
                                    <TouchableOpacity style={{ flex: 1, flexDirection: 'row', }} onPress={() => this.sortSelected('most')}>
                                        ...
                                        <View style={{ flex: 0.10 }}>
                                            {(this.state.sort && this.state.sort === 'most') ? <Image source={require('../../assets/images/tick.png')} style={{ width: '70%', height: '70%', resizeMode: 'cover', marginRight: 5 }} /> : null}
                                    ...
                                    <TouchableOpacity style={{ flex: 1, flexDirection: 'row', }} onPress={() => this.sortSelected('rating')}>
                                        ...
                                        <View style={{ flex: 0.10 }}>
                                            {(this.state.sort && this.state.sort === 'rating') ? <Image source={require('../../assets/images/tick.png')} style={{ width: '70%', height: '70%', resizeMode: 'cover', marginRight: 5 }} /> : null}
                ...
                {(this.state.filterType === 'Food') ?
                    <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: '' }}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute', backgroundColor: 'transparent', }}>
                            ...
                            <View style={{ flex: .38, backgroundColor: 'white' }}>
                                <View style={{ flex: 1, flexDirection: 'column', marginLeft: 25, marginRight: 10, marginTop: 10, marginBottom: 10, }}>
                ...
                {this.state.isAskDateTime ?
                    ...
                    <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flex: 1, position: 'absolute', backgroundColor: 'transparent' }}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'column', width: '100%', height: '100%', position: 'absolute', backgroundColor: 'transparent', alignSelf: 'center', justifyContent: 'center', alignItems: 'center' }}>
                            ...
                            ></TouchableOpacity>
                            <TouchableOpacity style={{ width: '90%', height: '30%', backgroundColor: 'white', borderRadius: 10, }}
                                ...
                                <View style={{ width: '100%', height: '100%', backgroundColor: 'white', borderRadius: 10, flexDirection: 'column', }}>
                                    ...
                                    <View style={{ justifyContent: 'flex-end' }}>
                                        <TouchableOpacity onPress={() => {
                                            ...
                                            <View style={{
                                                ...
                                                <Text
                                                    style={{ flex: .80, color: this.state.selectedTime ? "black" : '#8A8A8A', paddingLeft: 17, }}>{this.state.selectedTime ? this.state.selectedTime : "Please select time"}</Text>
                                    ...
                                    <View style={{ flex: 0.40, borderRadius: 8 }}>
                                        ...
                                        <View style={{ flex: 0.6, flexDirection: 'row', borderRadius: 8 }}>
                                            <TouchableOpacity style={{ flex: 0.5, alignSelf: 'center' }} onPress={() => this.disableDateTimePopup()}>
                                                <Text style={{ color: '#30445F', alignItems: 'center', justifyContent: 'center', alignSelf: 'center', fontSize: getDimen(.050) }}>Cancel</Text>
                                            </TouchableOpacity>
                                            <View style={{ height: '60%', width: 1, backgroundColor: '#8A8A8A', alignSelf: "center" }}></View>
                                            <TouchableOpacity style={{ flex: 0.5, alignSelf: 'center' }} onPress={() => this.submitAction()}>
    ...
    getRatingText = (average_rating, currencyName) => {
        if (average_rating === 5)
            return currencyName + '' + currencyName + '' + currencyName + '' + currencyName + '' + currencyName
        else if (average_rating >= 4 && average_rating < 5)
            return currencyName + '' + currencyName + '' + currencyName + '' + currencyName
        else if (average_rating >= 3 && average_rating < 4)
            return currencyName + '' + currencyName + '' + currencyName
